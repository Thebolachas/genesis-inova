<script lang="ts">
  ...
  // MÁGICA: Uma 'derived store'. Ela combina informações de outros stores.
  // Ela 'ouve' o selectedBlockId e a lista de blocks. Se um dos dois mudar,
  // ela automaticamente encontra e nos dá o objeto completo do bloco selecionado.
    ...
    <div class="controls-wrapper">
      {#if $selectedBlock.type === 'Header'}
        <div class="controle">
          ...
          <input type="text" bind:value={$selectedBlock.props.titulo}>
        </div>
        <div class="controle">
          <label>Cor de Fundo</label>
          <input type="color" bind:value={$selectedBlock.props.corDeFundo}>
        </div>
        {/if}
      ...
      {#if $selectedBlock.type === 'ProfileCard'}
        <div class="controle">
          <label>Nome</label>
          <input type="text" bind:value={$selectedBlock.props.nome}>
        </div>
        <div class="controle">
          <label>Biografia</label>
          <textarea rows="4" bind:value={$selectedBlock.props.bio}></textarea>
        </div>
        <div class="controle">
          <label>URL da Imagem</label>
          <input type="text" bind:value={$selectedBlock.props.imageUrl}>
        </div>
      {/if}
        ...
        {#each $selectedBlock.props.links as link, i}
          <div class="link-editor">
            ...
            <div class="controle">
              ...
              <input type="text" bind:value={link.text}>
            </div>
            <div class="controle">
              <label>URL</label>
...
<style>
  ...
  /* Estilos dos controles (reutilizados do V1) */
